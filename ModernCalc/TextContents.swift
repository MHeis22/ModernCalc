...
struct TextContents {
    ...
    static let builtinFunctions: [BuiltinFunction] = [
        ...
        .init(name: "binomdist", signature: "binomdist(k, n, p, [cumulative])", description: "Calculates the binomial distribution. The optional 4th argument selects the form: 0 (default) returns the PMF — the exact probability P(X = k); 1 returns the CDF — the cumulative probability P(X ≤ k). Example: 'binomdist(2, 5, 0.5, 0)' → PMF ≈ 0.3125; 'binomdist(2, 5, 0.5, 1)' → CDF = 0.5."),
        ...
        .init(name: "normdist", signature: "normdist(x, mean, stddev, [cumulative])", description: "Calculates the normal distribution. The optional 4th argument selects the form: 1 (default) returns the CDF — P(X ≤ x); 0 returns the PDF — the probability density at x. Example: 'normdist(1, 0, 1, 0)' → PDF ≈ 0.2420; 'normdist(1, 0, 1, 1)' → CDF ≈ 0.8413."),
        ...
        .init(name: "polyfit", signature: "polyfit(x, y, degree) or polyfit(matrix, degree)", description: "Fits a polynomial of the given degree. Accepts separate x and y vectors, or a 2-column matrix (col 1 = x, col 2 = y). Returns unit-aware coefficients."),
